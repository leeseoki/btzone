...
class Magestore_Megamenu_Adminhtml_MegamenuController extends Mage_Adminhtml_Controller_Action {
    ...
    public function saveAction() {
        if ($data = $this->getRequest()->getPost()) {
            ...
            if($data['menu_type'] == 6){
                ...
                $data['featured_products_box_title'] = '';
                $data['featured_categories'] = '';
                $data['featured_categories_box_title'] = '';
                $data['number_column'] = '';
                $data['header']='';
                $data['footer'] = '';
                $data['colum_category'] = '';
                $data['size_category'] = '';
                $data['products_box_title'] = '';
                $data['categories_box_title'] = '';
            }
            ...
            if (isset($_FILES['item_icon']['name']) && $_FILES['item_icon']['name'] != '') {
                try {
                    ...
                    $data['item_icon'] = $result['file'];
                } catch (Exception $e) {
                    $data['item_icon'] = $_FILES['item_icon']['name'];
                }
            }
    ...
    public function chooserMainProductsAction() {
        ...
        $block = $this->getLayout()->createBlock(
                'megamenu/adminhtml_megamenu_edit_tab_content_maincontent_sku', 'promo_widget_chooser_sku', array('js_form_object' => $request->getParam('form'),
                ));
        if ($block) {
    ...
    public function chooserFeaturedProductsAction() {
        $request = $this->getRequest();
        $block = $this->getLayout()->createBlock(
                'megamenu/adminhtml_megamenu_edit_tab_content_featureditem_sku', 'promo_widget_chooser_sku', array('js_form_object' => $request->getParam('form'),
                ));
        if ($block) {
            $this->getResponse()->setBody($block->toHtml());
        }
