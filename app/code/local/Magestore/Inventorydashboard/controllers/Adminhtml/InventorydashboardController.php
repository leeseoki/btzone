...
class Magestore_Inventorydashboard_Adminhtml_InventorydashboardController extends Mage_Adminhtml_Controller_Action {
    ...
    protected function _initAction() {
        $this->loadLayout()
                ->_setActiveMenu('inventorydashboard/inventorydashboard')
                ->_addBreadcrumb(
    ...
    public function editAction() {
        ...
        if ($model->getId() || $inventorydashboardId == 0) {
            ...
            $this->loadLayout();
            $this->_setActiveMenu('inventorydashboard/inventorydashboard');
    ...
    public function massDeleteAction() {
        ...
        if (!is_array($inventorydashboardIds)) {
            ...
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(
                        Mage::helper('adminhtml')->__('Total of %d record(s) were successfully deleted', count($inventorydashboardIds))
                );
            } catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
            }
    ...
    public function savechartAction() {
        ...
        $item_column = Mage::helper('inventorydashboard')->getItemColumn($tabId);
        $isEdit = false;
        $isCreateNew = false;
        if ($data && isset($data['group_type'])) {
            if (isset($data['item_id'])) {
                $results = Mage::helper('inventorydashboard')->getChartDataEdit($data);
            } else {
                $results = Mage::helper('inventorydashboard')->getChartData($data);
            }
            $name = $results['name'];
            $reportType = $results['reportType'];
            $chartType = $results['chartType'];
            $position = 0;
            if (isset($data['item_id']) && $id = $data['item_id']) {  //Edit
                ...
                $itemCollection = Mage::getModel('inventorydashboard/items')->getCollection()
                        ...
                        ->addFieldToFilter('item_column', $item_column)
                ...
                $model->setData('name', $name)
                        ...
                        ->setData('item_column', $item_column)
    ...
    public function changecharttypeAction() {
        ...
        foreach ($results as $result) {
            ...
            $response .= '<li style="float:left;margin-right:15px">
                <input type="radio" class="radio validate-one-required-by-name validation-passed" value="' . $result['chart_code'] . '" name="chart_type" title="' . $result['chart_code'] . '" ' . $checked . '/>
